function jobinfo = gen_training_examples4signal(PAR)
...
label_source  = PAR.label_source.(Signal_name);
info = PAR.info_genes.(PAR.Signal_name);


FN = PAR.FN;
fn_genome_config = FN.input.fn_genome_config ;
fn_training_blocks = FN.output.fn_training_blocks;
fn_genes_signals = FN.output.fn_genes_signals ;

input_sig = FN.input_sig.(Signal_name);
fn_candsites =  input_sig.fn_candsites;
fn_filter_settings = input_sig.fn_filter_settings;
fn_examples = input_sig.fn_examples;
fn_example_statistics = input_sig.fn_example_statistics;

run_locally = PAR.tasks.signals.run_locally.pos ;
...
%  diff_fields = compare_PAR(Signal,Old.P.Signal) ;
%  if ismember('Signal',Signal_name)
%    warning('label file exists, Signal parameters are differnt!')  
%    % fprintf('label file exists, Signal parameters are differnt!')  
%  end
...
if generate_examples
  fprintf(1,'generating labels\n')
  jobinfo = rproc_empty(0) ;
  
  P.fn_genes_signals = fn_genes_signals ;
  ...
  d=dir(sprintf('%scontig_*.pos',fn_candsites));
  num_files = length(d) ;
  assert(num_files==length(trivial_regions))
  if num_crashed>0, 
    num_crashed
    %return
  end 
...
modes = {'train','test'};
for mode=modes
  [filename, exists]=gen_example_filename(Signal,fn_filter_settings,fn_examples,mode{1},num_splits);
  if ~exists
   
    fprintf(1, 'SAVE EXAMPLES ....');
    ...
    if 0% ~run_locally
      ...
      [mem_req, time_req, opts] = rproc_mem_policy('save_examples',  0, RPROC.options) ;%RPROC.MEMREQ ;

      jobinfo = rproc('save_examples', D, mem_req, opts, time_req);
      [jobinfo_1, num_crashed]=rproc_wait(jobinfo, 60, 1, -1);
    else
      save_examples(blocks,split,P) ; num_crashed = 0 ;
    end
    if num_crashed>0
      return
    end
