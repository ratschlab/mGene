	...
	for (int chr_num=0; chr_num<g->num_contigs; chr_num++)
	{
		for (int strand=0; strand<2; strand++)
		{
			...
			* **************/	
			...
			save_score_pos(path_prefix, &score_name, &acc_label, num_scores, &acc_pos[0], acc_exp.size());

			create_dir(path_prefix, dn_output, chr_num, don, s);
			save_score_pos(path_prefix, &score_name, &don_label, num_scores, &don_pos[0], don_exp.size());
